...
static void int32_twostages_32(int32 *x,long long n)
{
  ...
  while (n > 0) {
    for (i = 0;i < 32;i += 8) {
      x0 = int32x8_load(&x[i]);
      x1 = int32x8_load(&x[i+32]);
      x2 = int32x8_load(&x[i+64]);
      x3 = int32x8_load(&x[i+96]);
...
{
  long long k,i;
  int32x8 x0, x1, x2, x3, x4, x5, x6, x7;

  for (k = 0;k + 8*q <= n;k += 8*q)
    for (i = k;i < k + q;i += 8) {
      x0 = int32x8_load(&x[i]);
      x1 = int32x8_load(&x[i+q]);
      x2 = int32x8_load(&x[i+2*q]);
      x3 = int32x8_load(&x[i+3*q]);
      x4 = int32x8_load(&x[i+4*q]);
      x5 = int32x8_load(&x[i+5*q]);
      x6 = int32x8_load(&x[i+6*q]);
      x7 = int32x8_load(&x[i+7*q]);
