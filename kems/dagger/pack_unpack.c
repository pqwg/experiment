...
void SABER_pack_2bit(uint8_t *bytes, uint16_t *data) {
	...
	{
		offset_data=4*j;
		bytes[j]=  (data[offset_data] & 0x03) |
                           ((data[offset_data + 1] & 0x03) <<2) |
                           ((data[offset_data + 2] & 0x03) <<4) |
                           ((data[offset_data + 3] & 0x03) <<6);
	}
}
void SABER_un_pack2bit(uint8_t *bytes, uint16_t *ar) {
	uint32_t j;
	uint32_t offset_data=0;
	...
	{
		offset_data=4*j;
		ar[offset_data] = bytes[j] & 0x03;
		ar[offset_data+1] = (bytes[j]>>2) & 0x03;
		ar[offset_data+2] = (bytes[j]>>4) & 0x03;
		ar[offset_data+3] = (bytes[j]>>6) & 0x03;
	}
}
...
// p = 512
static void POLVEC5122BS(uint8_t *bytes, uint16_t data[SABER_K][SABER_N]) {
	uint32_t i,j;
	uint32_t offset_data=0,offset_byte=0,offset_byte1=0;	
	...
	for(i=0;i<SABER_K;i++){
		...
		for(j=0;j<SABER_N/8;j++){ // every 72 bits
			offset_byte=offset_byte1+9*j;
			offset_data=8*j;
			bytes[offset_byte+0]= data[i][offset_data+0] & 0xff;
			bytes[offset_byte+1]= (data[i][offset_data+0]>>8) |
                                              ((data[i][offset_data+1] & 0x7f)<<1);
                        bytes[offset_byte+2]= (data[i][offset_data+1]>>7) |
                                              ((data[i][offset_data+2] & 0x3f)<<2);
                        bytes[offset_byte+3]= (data[i][offset_data+2]>>6) |
                                              ((data[i][offset_data+3] & 0x1f)<<3);
                        bytes[offset_byte+4]= (data[i][offset_data+3]>>5) |
                                              ((data[i][offset_data+4] & 0xf)<<4);
                        bytes[offset_byte+5]= (data[i][offset_data+4]>>4) | 
                                              ((data[i][offset_data+5]&0x7)<<5);
                        bytes[offset_byte+6]= (data[i][offset_data+5]>>3) |
                                              ((data[i][offset_data+6]&0x3)<<6);
                        bytes[offset_byte+7]= (data[i][offset_data+6]>>2) |
                                              ((data[i][offset_data+7]&0x1)<<7);
                        bytes[offset_byte+8]= data[i][offset_data+7]>>1;
		}
	}
}

// p = 512
static void BS2POLVEC512(const unsigned char *bytes, uint16_t data[SABER_K][SABER_N]){
	uint32_t i,j;
	uint32_t offset_data=0,offset_byte=0,offset_byte1=0;	
	...
	for(i=0;i<SABER_K;i++){
		...
		for(j=0;j<SABER_N/8;j++){
			...
			data[i][offset_data+0]= bytes[offset_byte+0] | ((bytes[offset_byte+1] & 0x1)<<8);
			data[i][offset_data+1]= (bytes[offset_byte+1]>>1)| ((bytes[offset_byte+2] & 0x3)<<7);
                        data[i][offset_data+2]= (bytes[offset_byte+2]>>2) | ((bytes[offset_byte+3] & 0x7) << 6);
                        data[i][offset_data+3]= (bytes[offset_byte+3]>>3) | ((bytes[offset_byte+4] & 0xf) << 5);
                        data[i][offset_data+4]= (bytes[offset_byte+4]>>4) | ((bytes[offset_byte+5] & 0x1f) << 4);
                        data[i][offset_data+5]= (bytes[offset_byte+5]>>5) | ((bytes[offset_byte+6] & 0x3f) << 3);
                        data[i][offset_data+6]= (bytes[offset_byte+6]>>6) | ((bytes[offset_byte+7] & 0x7f) << 2);
                        data[i][offset_data+7]= (bytes[offset_byte+7]>>7) | ((bytes[offset_byte+8] & 0xff) << 1);
		}
	}
}
