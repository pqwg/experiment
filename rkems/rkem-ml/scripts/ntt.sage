def precomp_sage():
    ...
    q = 10753
    mont = pow(2, 16, q)      # Modular exponentiation for modulo
    # mont = 1      # Modular exponentiation for modulo

    z = 0
    for i in range(1, q):
        z = i % q
        if multiplicative_order(Integers(q)(z)) == 512:
            break

    zetas = [0]*255
    for i in range(255):
        zetas[i] = (mont*pow(z, brv[i], q))%q # lift to integer representation
        if zetas[i] > q / 2:
            zetas[i] -= q
