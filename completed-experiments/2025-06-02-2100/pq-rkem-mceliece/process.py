...
LINE_REGEX = re.compile(
    r"\[\s*\d+\.\d+] wireguard: "
    r"(?P<role>client|server to) "
    r"\[(?P<client_public>[0-9a-f]+)\] "
    r"completed handshake in (?P<time>\d+) ns")
...
client_hs = [hs/1e6 for pk, roles in handshakes.items() for hs in roles["client"]]
server_hs = [hs/1e6 for pk, roles in handshakes.items() for hs in roles["server"]]
