...
again:
	...
	if (!peer) {
		...
		if (dev->flags & WGDEVICE_HAS_INITIATOR_PRIVATE_KEY) {
			if (sizeof(dev->initiator_private_key) < 3000) {
				mnl_attr_put(nlh, WGDEVICE_A_INITIATOR_PRIVATE_KEY, sizeof(dev->initiator_private_key), dev->initiator_private_key);
			} else {
				const u8* ptr = dev->initiator_private_key;
				mnl_attr_put(nlh, WGDEVICE_A_INITIATOR_PRIVATE_KEY_PTR, sizeof(&dev->initiator_private_key), &ptr);
			}
		}
		if (dev->flags & WGDEVICE_HAS_INITIATOR_PUBLIC_KEY) {
			if (sizeof(dev->initiator_public_key) < 3000) {
				mnl_attr_put(nlh, WGDEVICE_A_INITIATOR_PUBLIC_KEY, sizeof(dev->initiator_public_key), dev->initiator_public_key);
			} else {
				const u8* ptr = dev->initiator_public_key;
				mnl_attr_put(nlh, WGDEVICE_A_INITIATOR_PUBLIC_KEY_PTR, sizeof(&dev->initiator_public_key), &ptr);
			}
		}
		if (dev->flags & WGDEVICE_HAS_RESPONDER_PRIVATE_KEY) {
			if (sizeof(dev->responder_private_key) < 3000)
				mnl_attr_put(nlh, WGDEVICE_A_RESPONDER_PRIVATE_KEY, sizeof(dev->responder_private_key), dev->responder_private_key);
			else {
				const u8* ptr = dev->responder_private_key;
				mnl_attr_put(nlh, WGDEVICE_A_RESPONDER_PRIVATE_KEY_PTR, sizeof(&dev->responder_private_key), &ptr);
			}
		}
		if (dev->flags & WGDEVICE_HAS_RESPONDER_PUBLIC_KEY && KEM_PUBLIC_KEY_SIZE < 3000) 
			mnl_attr_put(nlh, WGDEVICE_A_RESPONDER_PUBLIC_KEY, sizeof(dev->responder_public_key), dev->responder_public_key);
		else if (dev->flags & WGDEVICE_HAS_RESPONDER_PUBLIC_KEY && KEM_PUBLIC_KEY_SIZE >= 3000) {
			const u8* ptr = dev->responder_public_key;
			mnl_attr_put(nlh, WGDEVICE_A_RESPONDER_PUBLIC_KEY_PTR, sizeof(&dev->responder_public_key), &ptr);
		}
	...
	for (peer = peer ? peer : dev->first_peer; peer; peer = peer->next_peer) {
		...
		if (!peer_nest)
			goto toobig_peers;
		if (peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY && sizeof(peer->initiator_public_key) < 3000) {
			if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE, WGPEER_A_INITIATOR_PUBLIC_KEY, sizeof(peer->initiator_public_key), peer->initiator_public_key))
			goto toobig_peers;
		}else if (peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY && sizeof(peer->initiator_public_key) >= 3000) {
			const u8* ptr = peer->initiator_public_key;
			if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE, WGPEER_A_INITIATOR_PUBLIC_KEY_PTR, sizeof(&peer->initiator_public_key), &ptr))
				goto toobig_peers;
		}
		if (peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY && KEM_PUBLIC_KEY_SIZE < 3000) {
			if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE, WGPEER_A_RESPONDER_PUBLIC_KEY, sizeof(peer->responder_public_key), peer->responder_public_key))
				goto toobig_peers;
		} else if (peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY && KEM_PUBLIC_KEY_SIZE >= 3000) {
			const u8* ptr = peer->responder_public_key;
			if (!mnl_attr_put_check(nlh, SOCKET_BUFFER_SIZE, WGPEER_A_RESPONDER_PUBLIC_KEY_PTR, sizeof(&peer->responder_public_key), &ptr)) { 
				puts("Failed here a1");
				goto toobig_peers;
			}
		}
...
static int parse_peer(const struct nlattr *attr, void *data)
{
	...
	case WGPEER_A_UNSPEC:
		break;
	case WGPEER_A_INITIATOR_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->initiator_public_key)) {
			memcpy(peer->initiator_public_key, mnl_attr_get_payload(attr), sizeof(peer->initiator_public_key));
			peer->flags |= WGPEER_HAS_INITIATOR_PUBLIC_KEY;
		}
		break;
	case WGPEER_A_RESPONDER_PUBLIC_KEY:
#if KEM_PUBLIC_KEY_SIZE < 3000
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->responder_public_key)) {
			memcpy(peer->responder_public_key, mnl_attr_get_payload(attr), sizeof(peer->responder_public_key));
			peer->flags |= WGPEER_HAS_RESPONDER_PUBLIC_KEY;
		}
#else
		memset(peer->responder_public_key, 'C', KEM_PUBLIC_KEY_SIZE);
		peer->flags |= WGPEER_HAS_RESPONDER_PUBLIC_KEY;
#endif
		break;
	case WGPEER_A_PRESHARED_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(peer->preshared_key)) {
			...
			if (!key_is_zero_psk(peer->preshared_key))
...
static int parse_peers(const struct nlattr *attr, void *data)
{
	...
	if (!ret)
		return ret;
	if (!(new_peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY) && !(new_peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY))
		return MNL_CB_ERROR;
	return MNL_CB_OK;
}
...
static int parse_device(const struct nlattr *attr, void *data)
{
	...
	case WGDEVICE_A_IFNAME:
		...
		break;
	case WGDEVICE_A_RESPONDER_PRIVATE_KEY:
#if KEM_PRIVATE_KEY_SIZE < 3000
		if (mnl_attr_get_payload_len(attr) == sizeof(device->responder_private_key)) {
			memcpy(device->responder_private_key, mnl_attr_get_payload(attr), sizeof(device->responder_private_key));
			device->flags |= WGDEVICE_HAS_RESPONDER_PRIVATE_KEY;
		}
#else
		puts("assuming responder private key");
		memset(device->responder_private_key, 0x41, sizeof(device->responder_private_key));
		device->flags |= WGDEVICE_HAS_RESPONDER_PRIVATE_KEY;
#endif
		break;
	case WGDEVICE_A_INITIATOR_PRIVATE_KEY:
		if (sizeof(device->initiator_private_key) < 3000) {
			if (mnl_attr_get_payload_len(attr) == sizeof(device->initiator_private_key)) {
				memcpy(device->initiator_private_key, mnl_attr_get_payload(attr), sizeof(device->initiator_private_key));
				device->flags |= WGDEVICE_HAS_INITIATOR_PRIVATE_KEY;
			}
		} else { 
			puts("assuming initiator private key");
			memset(device->initiator_private_key, 0x43, sizeof(device->initiator_private_key));
			device->flags |= WGDEVICE_HAS_INITIATOR_PRIVATE_KEY;
		}
		break;
	case WGDEVICE_A_RESPONDER_PUBLIC_KEY:
#if KEM_PUBLIC_KEY_SIZE < 3000
		if (mnl_attr_get_payload_len(attr) == sizeof(device->responder_public_key)) {
			memcpy(device->responder_public_key, mnl_attr_get_payload(attr), sizeof(device->responder_public_key));
			device->flags |= WGDEVICE_HAS_RESPONDER_PUBLIC_KEY;
		}
#else
		puts("assuming responder public key");
		memset(device->responder_public_key, 0x42, sizeof(device->responder_public_key));
		device->flags |= WGDEVICE_HAS_RESPONDER_PUBLIC_KEY;
#endif
		break;
	case WGDEVICE_A_INITIATOR_PUBLIC_KEY:
		if (mnl_attr_get_payload_len(attr) == sizeof(device->initiator_public_key)) {
			memcpy(device->initiator_public_key, mnl_attr_get_payload(attr), sizeof(device->initiator_public_key));
			device->flags |= WGDEVICE_HAS_INITIATOR_PUBLIC_KEY;
		}
		break;
...
static void coalesce_peers(struct wgdevice *device)
{
	...
	while (peer && peer->next_peer) {
		if (memcmp(peer->initiator_public_key, peer->next_peer->initiator_public_key, sizeof(peer->initiator_public_key)) &&
				memcmp(peer->responder_public_key, peer->next_peer->responder_public_key, sizeof(peer->responder_public_key))) {
			peer = peer->next_peer;
			continue;
		}
		if (!peer->first_allowedip) {
