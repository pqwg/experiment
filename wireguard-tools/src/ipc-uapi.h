...
#define MAX(a,b) (((a)>(b))?(a):(b))
	...
	if (dev->flags & WGDEVICE_HAS_RESPONDER_PRIVATE_KEY) {
		key_to_hex_responder_private(hex, dev->responder_private_key);
		fprintf(f, "responder_private_key=%s\n", hex);
		key_to_hex_responder_public(hex, dev->responder_public_key);
		fprintf(f, "responder_public_key=%s\n", hex);
	}
	if (dev->flags & WGDEVICE_HAS_INITIATOR_PRIVATE_KEY) {
		key_to_hex_initiator_private(hex, dev->initiator_private_key);
		fprintf(f, "initiator_private_key=%s\n", hex);
		key_to_hex_initiator_public(hex, dev->initiator_public_key);
		fprintf(f, "initiator_public_key=%s\n", hex);
	}
	...
	for_each_wgpeer(dev, peer) {
		if (peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY) {
			key_to_hex_initiator_public(hex, peer->initiator_public_key);
			fprintf(f, "peer_initiator_public_key=%s\n", hex);
		}
		if (peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY) {
			key_to_hex_responder_public(hex, peer->responder_public_key);
			fprintf(f, "peer_responder_public_key=%s\n", hex);
		}
		...
		if (peer->flags & WGPEER_HAS_PRESHARED_KEY) {
			key_to_hex_psk(hex, peer->preshared_key);
	...
	*out = dev = calloc(1, sizeof(*dev));
	...
	while (getline(&key, &line_buffer_len, f) > 0) {
		...
		*value++ = key[--line_len] = '\0';
		...
		if (!peer && !strcmp(key, "initiator_private_key")) {
			if (!key_from_hex_initiator_private(dev->initiator_private_key, value))
				break;
			dev->flags |= WGDEVICE_HAS_INITIATOR_PRIVATE_KEY;
		} else if (!peer && !strcmp(key, "initiator_public_key")) {
				if (!key_from_hex_initiator_public(dev->initiator_public_key, value))
					break;
				dev->flags |= WGDEVICE_HAS_INITIATOR_PUBLIC_KEY;
		} else if (!peer && !strcmp(key, "responder_private_key")) {
			if (!key_from_hex_responder_private(dev->responder_private_key, value))
				break;
			dev->flags |= WGDEVICE_HAS_RESPONDER_PRIVATE_KEY;
		} else if (!peer && !strcmp(key, "responder_public_key")) {
			if (!key_from_hex_responder_public(dev->responder_public_key, value))
				break;
			dev->flags |= WGDEVICE_HAS_RESPONDER_PUBLIC_KEY;
		...
		} else if (!strcmp(key, "initiator_public_key")) {
			...
			peer = new_peer;
			if (!key_from_hex_initiator_public(peer->initiator_public_key, value))
				break;
			peer->flags |= WGPEER_HAS_INITIATOR_PUBLIC_KEY;
		} else if (!strcmp(key, "responder_public_key")) {
			struct wgpeer *new_peer = calloc(1, sizeof(*new_peer));
			...
			if (peer)
				...
				dev->first_peer = new_peer;
			peer = new_peer;
			if (!key_from_hex_responder_public(peer->responder_public_key, value))
				break;
			peer->flags |= WGPEER_HAS_RESPONDER_PUBLIC_KEY;
		} else if (peer && !strcmp(key, "preshared_key")) {
			if (!key_from_hex_psk(peer->preshared_key, value))
				break;
