...
#define KEY_TYPE_RESPONDER 0
#define KEY_TYPE_INITIATOR 1
#define KEY_TYPE_PSK 	   2

#define KEY_TYPE_PUBLIC    0
#define KEY_TYPE_PRIVATE   1U << 4

static char *key(const uint8_t key[], int which_key)
{
	static char base64[WG_KEY_LEN_HEX_MAX];
	switch (which_key & 0xF)
	{
	case KEY_TYPE_RESPONDER:
		if (which_key & KEY_TYPE_PRIVATE)
			key_to_base64_responder_private(base64, key);
		else
			key_to_base64_responder_public(base64, key);
		break;
	case KEY_TYPE_INITIATOR:
		if (which_key & KEY_TYPE_PRIVATE)
			key_to_base64_initiator_private(base64, key);
		else
			key_to_base64_initiator_public(base64, key);
		break;
	case KEY_TYPE_PSK:
		key_to_base64_psk(base64, key);
		break;
	default:
...
static const char *maybe_key(const uint8_t maybe_key[], int which_key, bool have_it)
{
	if (!have_it)
		return "(none)";
	return key(maybe_key, which_key);
}

static const char *masked_key(const uint8_t masked_key[], int which_key)
{
	static char base64[WG_KEY_LEN_HEX_MAX];
	// const char *var = getenv("WG_HIDE_KEYS");

	// if (!var || 0 == strcmp(var, "never"))
	// 	return "hidden";

	switch (which_key & 0xF)
	{
	case KEY_TYPE_RESPONDER:
		key_to_base64_responder_private(base64, masked_key);
		break;
	case KEY_TYPE_INITIATOR:
		key_to_base64_initiator_private(base64, masked_key);
		break;
	case KEY_TYPE_PSK:
		key_to_base64_psk(base64, masked_key);
		break;
	default:
		return "ERROR";
	}
...
static void pretty_print(struct wgdevice *device)
{
	...
	if (device->flags & WGDEVICE_HAS_INITIATOR_PUBLIC_KEY)
		terminal_printf("  " TERMINAL_BOLD "initiator public key" TERMINAL_RESET ": %s\n", key(device->initiator_public_key, KEY_TYPE_INITIATOR));
	if (device->flags & WGDEVICE_HAS_RESPONDER_PUBLIC_KEY)
		terminal_printf("  " TERMINAL_BOLD "responder public key" TERMINAL_RESET ": %s\n", key(device->responder_public_key, KEY_TYPE_RESPONDER));
	if (device->flags & WGDEVICE_HAS_INITIATOR_PRIVATE_KEY)
		terminal_printf("  " TERMINAL_BOLD "initiator private key" TERMINAL_RESET ": %s\n", masked_key(device->initiator_private_key, KEY_TYPE_INITIATOR));
	if (device->flags & WGDEVICE_HAS_RESPONDER_PRIVATE_KEY)
		terminal_printf("  " TERMINAL_BOLD "responder private key" TERMINAL_RESET ": %s\n", masked_key(device->responder_private_key, KEY_TYPE_RESPONDER));
	...
	for_each_wgpeer(device, peer) {
		terminal_printf(TERMINAL_FG_YELLOW TERMINAL_BOLD "peer" TERMINAL_RESET ": \n");
		terminal_printf("  " TERMINAL_BOLD "flags: %04x\n", peer->flags);
		if (peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY)
			 terminal_printf("  " TERMINAL_BOLD "initiator public key: %s" TERMINAL_RESET "\n", key(peer->initiator_public_key, KEY_TYPE_INITIATOR));
		if (peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY)
			 terminal_printf("  " TERMINAL_BOLD "responder public key: %s" TERMINAL_RESET "\n", key(peer->responder_public_key, KEY_TYPE_RESPONDER));
		if (peer->flags & WGPEER_HAS_PRESHARED_KEY)
			terminal_printf("  " TERMINAL_BOLD "preshared key" TERMINAL_RESET ": %s\n", masked_key(peer->preshared_key, KEY_TYPE_PSK));
...
static void dump_print(struct wgdevice *device, bool with_interface)
{
	...
	printf("%s\t", maybe_key(device->initiator_private_key, KEY_TYPE_INITIATOR | KEY_TYPE_PRIVATE, device->flags & WGDEVICE_HAS_INITIATOR_PRIVATE_KEY));
	printf("%s\t", maybe_key(device->initiator_public_key, KEY_TYPE_INITIATOR, device->flags & WGDEVICE_HAS_INITIATOR_PUBLIC_KEY));
	printf("%s\t", maybe_key(device->responder_private_key, KEY_TYPE_RESPONDER | KEY_TYPE_PRIVATE, device->flags & WGDEVICE_HAS_RESPONDER_PRIVATE_KEY));
	printf("%s\t", maybe_key(device->responder_public_key, KEY_TYPE_RESPONDER, device->flags & WGDEVICE_HAS_RESPONDER_PUBLIC_KEY));
	...
	for_each_wgpeer(device, peer) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\t", maybe_key(peer->initiator_public_key, KEY_TYPE_INITIATOR, peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY));
		printf("%s\t", maybe_key(peer->responder_public_key, KEY_TYPE_RESPONDER, peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY));
		printf("%s\t", maybe_key(peer->preshared_key, KEY_TYPE_PSK, peer->flags & WGPEER_HAS_PRESHARED_KEY));
		if (peer->endpoint.addr.sa_family == AF_INET || peer->endpoint.addr.sa_family == AF_INET6)
...
static bool ugly_print(struct wgdevice *device, const char *param, bool with_interface)
{
	...
	if (!strcmp(param, "initiator-public-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->initiator_public_key, KEY_TYPE_INITIATOR, device->flags & WGDEVICE_HAS_INITIATOR_PUBLIC_KEY));
	} else if (!strcmp(param, "responder-public-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->responder_public_key, KEY_TYPE_RESPONDER, device->flags & WGDEVICE_HAS_RESPONDER_PUBLIC_KEY));
	} else if (!strcmp(param, "initiator-private-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->initiator_private_key, KEY_TYPE_INITIATOR | KEY_TYPE_PRIVATE, device->flags & WGDEVICE_HAS_INITIATOR_PRIVATE_KEY));
	} else if (!strcmp(param, "responder-private-key")) {
		if (with_interface)
			printf("%s\t", device->name);
		printf("%s\n", maybe_key(device->responder_private_key, KEY_TYPE_RESPONDER | KEY_TYPE_PRIVATE, device->flags & WGDEVICE_HAS_RESPONDER_PRIVATE_KEY));
		...
		for_each_wgpeer(device, peer) {
			...
			printf("%s\t", maybe_key(peer->initiator_public_key, KEY_TYPE_INITIATOR, peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY));
			printf("%s\t", maybe_key(peer->responder_public_key, KEY_TYPE_RESPONDER, peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY));
			printf("%llu\n", (unsigned long long)peer->last_handshake_time.tv_sec);
		}
		...
		for_each_wgpeer(device, peer) {
			...
			printf("%s\t", maybe_key(peer->initiator_public_key, KEY_TYPE_INITIATOR, peer->flags & WGPEER_HAS_INITIATOR_PUBLIC_KEY));
			printf("%s\t", maybe_key(peer->responder_public_key, KEY_TYPE_RESPONDER, peer->flags & WGPEER_HAS_RESPONDER_PUBLIC_KEY));
			printf("%s\n", maybe_key(peer->preshared_key, KEY_TYPE_PSK, peer->flags & WGPEER_HAS_PRESHARED_KEY));
		}
