...
static void show_usage(FILE *file)
{
	...
	for (size_t i = 0; i < sizeof(subcommands) / sizeof(subcommands[0]) - 3; ++i)
...
int main(int argc, const char *argv[])
{
	...
	if (argc == 2 && (!strcmp(argv[1], "-v") || !strcmp(argv[1], "--version") || !strcmp(argv[1], "version"))) {
		printf("wireguard-tools v%s - https://git.zx2c4.com/wireguard-tools/ - KEM: %s, RKEM: %s\n", WIREGUARD_TOOLS_VERSION, KEM_NAME, RKEM_NAME);
		return 0;
	}
